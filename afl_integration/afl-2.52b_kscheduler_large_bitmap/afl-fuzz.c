...
int energy_cmp(const void *p1, const void *p2)
{
  ...
  if (isless(e1->energy, e2->energy))
    return 1;
  if (isgreater(e1->energy, e2->energy))
    return -1;
  return 0;
}
...
static double calculate_score(struct queue_entry *q, double thres_energy, u32 perf_score)
{
  ...
  if (energy > HAVOC_MAX_MULT * 100)
    energy = HAVOC_MAX_MULT * 100;
...
static u8 fuzz_one(char **argv)
{
  ...
  if (q->exec_us * 0.05 > avg_us_fast)
    init_perf_score = 0;
  else if (q->exec_us * 0.075 > avg_us_fast)
    init_perf_score = 1;
  else if (q->exec_us * 0.1 > avg_us_fast)
    init_perf_score = 10;
  else if (q->exec_us * 0.25 > avg_us_fast)
    init_perf_score = 25;
  else if (q->exec_us * 0.5 > avg_us_fast)
    init_perf_score = 50;
  else if (q->exec_us * 0.75 > avg_us_fast)
    init_perf_score = 75;
  else if (q->exec_us * 4 < avg_us_fast)
    init_perf_score = 300;
  else if (q->exec_us * 3 < avg_us_fast)
    init_perf_score = 200;
  else if (q->exec_us * 2 < avg_us_fast)
    init_perf_score = 150;

  if (init_perf_score <= 75)
    queue_cur->trim_done = 1;

  if (init_perf_score == 0)
    goto abandon_entry;
  ...
  if (fpclassify(perf_score) == FP_ZERO)
    goto abandon_entry;
...
havoc_stage:
  ...
  if (!splice_cycle)
  {
    ...
    if (stage_max < 2 * HAVOC_MIN)
      stage_max = 2 * HAVOC_MIN;
